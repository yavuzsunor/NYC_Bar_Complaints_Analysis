...
from
	...
	max(cast(DefMon6 as int)) Def6, cast(avg(OccpMon6) as numeric(18,2)) AvgOccp6,
	max(cast(DefMon6_3 as int)) Def6_3, cast(avg(OccpMon6_3) as numeric(18,2)) AvgOccp6_3,
	max(cast(DefMon12 as int)) Def12, cast(avg(OccpMon12) as numeric(18,2)) AvgOccp12,
	max(cast(DefMon12_6 as int)) Def12_6, cast(avg(OccpMon12_6) as numeric(18,2)) AvgOccp12_6,
	max(cast(DefMon18 as int)) Def18, cast(avg(OccpMon18) as numeric(18,2)) AvgOccp18,
	max(cast(DefMon18_6 as int)) Def18_6, cast(avg(OccpMon18_6) as numeric(18,2)) AvgOccp18_6
		...
		left join
			...
			from
				...
				from 
					...
					and c.Id in (select CustomerTransactionId from dbo.x_indus_badr_final) 
					and a.CreditTypeId=b.Id) t ) t2
			group by CustomerId, Description) t3
		on b.CustomerId = t3.CustomerId
		where b.Id in (select CustomerTransactionId from dbo.x_indus_badr_final) ) t4
