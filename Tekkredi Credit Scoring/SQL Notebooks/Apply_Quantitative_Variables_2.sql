	...
	from
		...
		from
			...
			from
				...
				from dbo.Apply_2018_Def a, dbo.Apply_2018 b
				where a.CustomerId = b.CustomerId
				and a.ApplyId = b.ApplyId
				and a.ApplyId = '0C730A8E-8686-4872-BF24-06A4013C69EC'
...
select * 
into dbo.QuantVar_All_2
from dbo.QuantVar_2014_2
union
select * from dbo.QuantVar_2015_2
union
select * from dbo.QuantVar_2016_2
union
select * from dbo.QuantVar_2017_2
union
select * from dbo.QuantVar_2018_2



declare @var1_zero as float
select @var1_zero = min(FullAvg30Mon6) from dbo.QuantVar_All_2 where FullAvg30Mon6 <> 0

declare @var3_zero as float 
select @var3_zero = min(FullAvg90Mon18)  from dbo.QuantVar_All_2 where FullAvg90Mon18 <> 0

declare @var4_zero as float 
select @var4_zero = min(ConsAvg60Mon6Active) from dbo.QuantVar_All_2 where ConsAvg60Mon6Active <> 0

declare @var5_zero as float 
select @var5_zero = min(FullAvgLimitActive) from dbo.QuantVar_All_2 where FullAvgLimitActive <> 0

declare @var7_zero as float 
select @var7_zero = min(ODAvg90Mon6) from dbo.QuantVar_All_2 where ODAvg90Mon6 <> 0

select ApplyId, ApplyTime, CustomerId, Def,

cast(case when FullAvg30Mon6 = 0 then FullAvg30Mon18/@var1_zero else FullAvg30Mon18/FullAvg30Mon6 end as numeric(18,2)) var1,
cast(FullAvg30Mon18 as numeric(18,2)) FullAvg30Mon18,
cast(FullAvg30Mon6 as numeric(18,2)) FullAvg30Mon6,

Score var2, 

cast(case when FullAvg90Mon18 = 0 then 1/@var3_zero else 1/FullAvg90Mon18 end as numeric(18,2)) var3,
cast(FullAvg90Mon18 as numeric(18,2)) FullAvg90Mon18,

cast(log(case when ConsAvg60Mon6Active = 0 then @var4_zero else ConsAvg60Mon6Active end + 1) as numeric(18,2)) var4,
cast(ConsAvg60Mon6Active as numeric(18,2)) ConsAvg60Mon6Active, 

cast(log(case when FullAvgLimitActive = 0 then FullAvgDebtActive/@var5_zero else FullAvgDebtActive/FullAvgLimitActive end + 1) as numeric(18,2)) var5,
cast(FullAvgLimitActive as numeric(18,2)) FullAvgLimitActive, 
cast(FullAvgDebtActive as numeric(18,2)) FullAvgDebtActive, 

cast(CredAvgOnTimeMon3 as numeric(18,2)) var6, 

cast(case when ODAvg90Mon6 = 0 then 1/@var7_zero else 1/ODAvg90Mon6 end as numeric(18,2)) var7,
cast(ODAvg90Mon6 as numeric(18,2)) ODAvg90Mon6

into dbo.Sample2IndusVar
from 
	(
	select a.ApplyId, a.ApplyTime, a.CustomerId,  
	case when a.ConsCred = 1 then 1  
		 when a.CredCard = 1 then 1
		 when a.Overdraft = 1 then 1
		 when a.Mortgage = 1 then 1
		 when a.Car = 1 then 1 else 0 end Def,
	cast (case when FullAvg30Mon6 is NULL then (select avg(FullAvg30Mon6) from dbo.QuantVar_All_2) else FullAvg30Mon6 end as numeric(18,2)) FullAvg30Mon6, 
	cast (case when FullAvg30Mon18 is NULL then (select avg(FullAvg30Mon18) from dbo.QuantVar_All_2) else FullAvg30Mon18 end as numeric(18,2)) FullAvg30Mon18,
	cast (case when Score is NULL then (select avg(Score) from dbo.QuantVar_All_2) else Score end as numeric(18,2)) Score, 
	cast (case when FullAvg90Mon18 is NULL then (select avg(FullAvg90Mon18) from dbo.QuantVar_All_2) else FullAvg90Mon18 end as numeric(18,2)) FullAvg90Mon18, 
	cast (case when ConsAvg60Mon6Active is NULL then (select avg(ConsAvg60Mon6Active) from dbo.QuantVar_All_2) else ConsAvg60Mon6Active end as numeric(18,2)) ConsAvg60Mon6Active, 
	cast (case when FullAvgLimitActive is NULL then (select avg(FullAvgLimitActive) from dbo.QuantVar_All_2) else FullAvgLimitActive end as numeric(18,2)) FullAvgLimitActive,
	cast (case when FullAvgDebtActive is NULL then (select avg(FullAvgDebtActive) from dbo.QuantVar_All_2) else FullAvgDebtActive end as numeric(18,2)) FullAvgDebtActive, 
	cast (case when CredAvgOnTimeMon3 is NULL then (select avg(CredAvgOnTimeMon3) from dbo.QuantVar_All_2) else CredAvgOnTimeMon3 end as numeric(18,2)) CredAvgOnTimeMon3, 
	cast (case when ODAvg90Mon6 is NULL then (select avg(ODAvg90Mon6) from dbo.QuantVar_All_2) else ODAvg90Mon6 end as numeric(18,2)) ODAvg90Mon6
	from dbo.Full_Def_2 a, dbo.QuantVar_All_2 b
	where a.ApplyId = b.ApplyId
	and a.ApplyTime = b.ApplyTime
	and a.CustomerId = b.CustomerId
	and FullAvgDebtActive >= 0
	) t
...
case when a.ConsCred = 1 then 1  
	 ...
	 when a.Mortgage = 1 then 1
	 when a.Car = 1 then 1 else 0 end Def,
ApplyHour, ApplyMonth, Age, Gender, CityName, MonthlyCanBePaid, Income,
...
b.*
into dbo.Sample3FullVar
from dbo.Full_Def_2 a, dbo.QuantVar_All_2 b, dbo.Full_Apply c
where a.ApplyId = b.ApplyId
and a.ApplyTime = b.ApplyTime
and a.CustomerId = b.CustomerId
and a.ApplyId = c.ApplyId
and a.ApplyTime = c.ApplyTime
and a.CustomerId = c.CustomerId
and FullAvgDebtActive >= 0



select Def,ApplyId,ApplyTime,CustomerId,
...
Score,ConsCountLoan,CredCountLoan,ODCountLoan,MorgCountLoan,CarCountLoan,FullAvgLimit,ConsAvgLimit,CredAvgLimit,ODAvgLimit,MorgAvgLimit,CarAvgLimit,FullAvgDebt,
ConsAvgDebt,CredAvgDebt,ODAvgDebt,MorgAvgDebt,CarAvgDebt,FullAvg30Mon3,ConsAvg30Mon3,CredAvg30Mon3,ODAvg30Mon3,MorgAvg30Mon3,CarAvg30Mon3,FullAvg60Mon3,ConsAvg60Mon3,CredAvg60Mon3,ODAvg60Mon3,
MorgAvg60Mon3,CarAvg60Mon3,FullAvg90Mon3,ConsAvg90Mon3,CredAvg90Mon3,ODAvg90Mon3,MorgAvg90Mon3,CarAvg90Mon3,FullAvgOnTimeMon3,ConsAvgOnTimeMon3,CredAvgOnTimeMon3,ODAvgOnTimeMon3,MorgAvgOnTimeMon3,
CarAvgOnTimeMon3,FullAvg30Mon6,ConsAvg30Mon6,CredAvg30Mon6,ODAvg30Mon6,MorgAvg30Mon6,CarAvg30Mon6,FullAvg60Mon6,ConsAvg60Mon6,CredAvg60Mon6,ODAvg60Mon6,MorgAvg60Mon6,CarAvg60Mon6,FullAvg90Mon6,
ConsAvg90Mon6,CredAvg90Mon6,ODAvg90Mon6,MorgAvg90Mon6,CarAvg90Mon6,FullAvgOnTimeMon6,ConsAvgOnTimeMon6,CredAvgOnTimeMon6,ODAvgOnTimeMon6,MorgAvgOnTimeMon6,CarAvgOnTimeMon6,FullAvg30Mon12,
ConsAvg30Mon12,CredAvg30Mon12,ODAvg30Mon12,MorgAvg30Mon12,CarAvg30Mon12,FullAvg60Mon12,ConsAvg60Mon12,CredAvg60Mon12,ODAvg60Mon12,MorgAvg60Mon12,CarAvg60Mon12,FullAvg90Mon12,ConsAvg90Mon12,
CredAvg90Mon12,ODAvg90Mon12,MorgAvg90Mon12,CarAvg90Mon12,FullAvgOnTimeMon12,ConsAvgOnTimeMon12,CredAvgOnTimeMon12,ODAvgOnTimeMon12,MorgAvgOnTimeMon12,CarAvgOnTimeMon12,FullAvg30Mon18,ConsAvg30Mon18,
CredAvg30Mon18,ODAvg30Mon18,MorgAvg30Mon18,CarAvg30Mon18,FullAvg60Mon18,ConsAvg60Mon18,CredAvg60Mon18,ODAvg60Mon18,MorgAvg60Mon18,CarAvg60Mon18,FullAvg90Mon18,ConsAvg90Mon18,CredAvg90Mon18,
ODAvg90Mon18,MorgAvg90Mon18,CarAvg90Mon18,FullAvgOnTimeMon18,ConsAvgOnTimeMon18,CredAvgOnTimeMon18,ODAvgOnTimeMon18,MorgAvgOnTimeMon18,CarAvgOnTimeMon18
into dbo.Sample4
from dbo.Sample3FullVar



select max(FullAvg60Mon6 / FullAvg60Mon18) from dbo.Sample4 where FullAvg60Mon18 <> 0




select 
ConsCountLoan*1.0 / (ISNULL(ConsCountLoan, 0)+ISNULL(CredCountLoan, 0)+ISNULL(ODCountLoan, 0)+ISNULL(MorgCountLoan, 0)+ISNULL(CarCountLoan, 0))*1.0 RatioCons,
CredCountLoan*1.0 / (ISNULL(ConsCountLoan, 0)+ISNULL(CredCountLoan, 0)+ISNULL(ODCountLoan, 0)+ISNULL(MorgCountLoan, 0)+ISNULL(CarCountLoan, 0))*1.0 RatioCred,
ODCountLoan*1.0 / (ISNULL(ConsCountLoan, 0)+ISNULL(CredCountLoan, 0)+ISNULL(ODCountLoan, 0)+ISNULL(MorgCountLoan, 0)+ISNULL(CarCountLoan, 0))*1.0 RatioOD,
MorgCountLoan*1.0 / (ISNULL(ConsCountLoan, 0)+ISNULL(CredCountLoan, 0)+ISNULL(ODCountLoan, 0)+ISNULL(MorgCountLoan, 0)+ISNULL(CarCountLoan, 0))*1.0 RatioMorg,
CarCountLoan*1.0 / (ISNULL(ConsCountLoan, 0)+ISNULL(CredCountLoan, 0)+ISNULL(ODCountLoan, 0)+ISNULL(MorgCountLoan, 0)+ISNULL(CarCountLoan, 0))*1.0 RatioCar,

FullAvgDebt / NULLIF(FullAvgLimit, 0) RatioFullDebtLimit,
ConsAvgDebt / NULLIF(ConsAvgLimit, 0) RatioConsDebtLimit,
CredAvgDebt / NULLIF(CredAvgLimit, 0) RatioCredDebtLimit,
ODAvgDebt / NULLIF(ODAvgLimit, 0) RatioODDebtLimit,
MorgAvgDebt / NULLIF(MorgAvgLimit, 0) RatioMorgDebtLimit,
CarAvgDebt / NULLIF(CarAvgLimit, 0) RatioCarDebtLimit,

FullAvgDebt / NULLIF(Income, 0) DebttoIncome,
MonthlyCanBePaid / NULLIF(Income, 0) MonPaidtoIncome,

case when FullAvg30Mon6 = 0 then 1 else FullAvg30Mon3 / FullAvg30Mon6 end Ratio30of3to6, 
case when FullAvg30Mon12 = 0 then 1 else FullAvg30Mon3 / FullAvg30Mon12 end Ratio30of3to12, 
case when FullAvg30Mon18 = 0 then 1 else FullAvg30Mon3 / FullAvg30Mon18 end Ratio30of3to18,  
case when FullAvg30Mon12 = 0 then 1 else FullAvg30Mon6 / FullAvg30Mon12 end Ratio30of6to12, 
case when FullAvg30Mon18 = 0 then 1 else FullAvg30Mon6 / FullAvg30Mon18 end Ratio30of6to18, 
case when FullAvg30Mon18 = 0 then 1 else FullAvg30Mon12 / FullAvg30Mon18 end Ratio30of12to18,

case when FullAvg60Mon6 = 0 then 1 else FullAvg60Mon3 / FullAvg60Mon6 end Ratio60of3to6, 
case when FullAvg60Mon12 = 0 then 1 else FullAvg60Mon3 / FullAvg60Mon12 end Ratio60of3to12, 
case when FullAvg60Mon18 = 0 then 1 else FullAvg60Mon3 / FullAvg60Mon18 end Ratio60of3to18,  
case when FullAvg60Mon12 = 0 then 1 else FullAvg60Mon6 / FullAvg60Mon12 end Ratio60of6to12, 
case when FullAvg60Mon18 = 0 then 1 else FullAvg60Mon6 / FullAvg60Mon18 end Ratio60of6to18, 
case when FullAvg60Mon18 = 0 then 1 else FullAvg60Mon12 / FullAvg60Mon18 end Ratio60of12to18,

case when FullAvg90Mon6 = 0 then 1 else FullAvg90Mon3 / FullAvg90Mon6 end Ratio90of3to6, 
case when FullAvg90Mon12 = 0 then 1 else FullAvg90Mon3 / FullAvg90Mon12 end Ratio90of3to12, 
case when FullAvg90Mon18 = 0 then 1 else FullAvg90Mon3 / FullAvg90Mon18 end Ratio90of3to18,  
case when FullAvg90Mon12 = 0 then 1 else FullAvg90Mon6 / FullAvg90Mon12 end Ratio90of6to12, 
case when FullAvg90Mon18 = 0 then 1 else FullAvg90Mon6 / FullAvg90Mon18 end Ratio90of6to18, 
