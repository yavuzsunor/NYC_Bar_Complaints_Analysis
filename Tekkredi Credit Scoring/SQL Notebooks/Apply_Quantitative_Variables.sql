	...
	from
		...
		from
			...
			from
				...
				from dbo.Apply_2017_Def a, dbo.Apply_2017 b
...
select * 
into dbo.QuantVar_All
from dbo.QuantVar_2014
union
select * from dbo.QuantVar_2015
union
select * from dbo.QuantVar_2016
union
select * from dbo.QuantVar_2017


declare @var1_zero as float
select @var1_zero = min(FullAvg30Mon6) from dbo.QuantVar_All where FullAvg30Mon6 <> 0

declare @var3_zero as float 
select @var3_zero = min(FullAvg90Mon18)  from dbo.QuantVar_All where FullAvg90Mon18 <> 0

declare @var4_zero as float 
select @var4_zero = min(ConsAvg60Mon6Active) from dbo.QuantVar_All where ConsAvg60Mon6Active <> 0

declare @var5_zero as float 
select @var5_zero = min(FullAvgLimitActive) from dbo.QuantVar_All where FullAvgLimitActive <> 0

declare @var7_zero as float 
select @var7_zero = min(ODAvg90Mon6) from dbo.QuantVar_All where ODAvg90Mon6 <> 0
...
from 
	...
	cast (case when FullAvg30Mon6 is NULL then (select avg(FullAvg30Mon6) from dbo.QuantVar_All) else FullAvg30Mon6 end as numeric(18,2)) FullAvg30Mon6, 
	cast (case when FullAvg30Mon18 is NULL then (select avg(FullAvg30Mon18) from dbo.QuantVar_All) else FullAvg30Mon18 end as numeric(18,2)) FullAvg30Mon18,
	cast (case when Score is NULL then (select avg(Score) from dbo.QuantVar_All) else Score end as numeric(18,2)) Score, 
	cast (case when FullAvg90Mon18 is NULL then (select avg(FullAvg90Mon18) from dbo.QuantVar_All) else FullAvg90Mon18 end as numeric(18,2)) FullAvg90Mon18, 
	cast (case when ConsAvg60Mon6Active is NULL then (select avg(ConsAvg60Mon6Active) from dbo.QuantVar_All) else ConsAvg60Mon6Active end as numeric(18,2)) ConsAvg60Mon6Active, 
	cast (case when FullAvgLimitActive is NULL then (select avg(FullAvgLimitActive) from dbo.QuantVar_All) else FullAvgLimitActive end as numeric(18,2)) FullAvgLimitActive,
	cast (case when FullAvgDebtActive is NULL then (select avg(FullAvgDebtActive) from dbo.QuantVar_All) else FullAvgDebtActive end as numeric(18,2)) FullAvgDebtActive, 
	cast (case when CredAvgOnTimeMon3 is NULL then (select avg(CredAvgOnTimeMon3) from dbo.QuantVar_All) else CredAvgOnTimeMon3 end as numeric(18,2)) CredAvgOnTimeMon3, 
	cast (case when ODAvg90Mon6 is NULL then (select avg(ODAvg90Mon6) from dbo.QuantVar_All) else ODAvg90Mon6 end as numeric(18,2)) ODAvg90Mon6
	from dbo.Full_Def a, dbo.QuantVar_All b
