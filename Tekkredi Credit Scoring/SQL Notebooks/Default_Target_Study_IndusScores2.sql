select * into dbo.IndusScore_New
from
	(select t_1.CustomerApplyId, t_1.risk_value, t_2.* 
	from dbo.x_indus_badr_final t_1

	left join

	(select CustomerTransactionId, TransTime, CustomerId, 
	max(cast(DefLast3 as int)) DefLast3,
	max(cast(DefMon3 as int)) Def3, cast(avg(OccpMon3) as numeric(18,2)) AvgOccp3,
	max(cast(DefMon3_6 as int)) Def3_6, cast(avg(OccpMon3_6) as numeric(18,2)) AvgOccp3_6,
	max(cast(DefMon6_9 as int)) Def6_9, cast(avg(OccpMon6_9) as numeric(18,2)) AvgOccp6_9,
	max(cast(DefMon9_12 as int)) Def9_12, cast(avg(OccpMon9_12) as numeric(18,2)) AvgOccp9_12,
	max(cast(DefMon12_15 as int)) Def12_15, cast(avg(OccpMon12_15) as numeric(18,2)) AvgOccp12_15,
	max(cast(DefMon15_18 as int)) Def15_18, cast(avg(OccpMon15_18) as numeric(18,2)) AvgOccp15_18
	...
	from
		...
		case when RIGHT(LEFT(t3.Performance, (DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 3) like '%K%' 
		or RIGHT(LEFT(t3.Performance, (DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 3) like '%I%' 
		or RIGHT(LEFT(t3.Performance, (DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 3) like '%3%'
		or RIGHT(LEFT(t3.Performance, (DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 3) like '%4%' then 1 else 0 end DefLast3,

		RIGHT(LEFT(t3.Performance, (DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 3) Last3,
		RIGHT(LEFT(t3.Performance, (DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 6) Last6,
		RIGHT(LEFT(t3.Performance, (DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 12) Last12,
		RIGHT(LEFT(t3.Performance, (DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 18) Last18,		
		...
		or LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 3) like '%3%'
		or LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 3) like '%4%' then 1 else 0 end DefMon3,
		...
		RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 6), 3) Mon3_6,
		case when RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 6), 3) like '%K%' 
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 6), 3) like '%I%' 
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 6), 3) like '%3%'
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 6), 3) like '%4%' then 1 else 0 end DefMon3_6,
		cast(LEN(REPLACE(REPLACE(RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 6), 3), '/', ''), '.', '')) as float)/
		cast(LEN(RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 6), 3)) as float) OccpMon3_6,

		RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 9), 3) Mon6_9,
		case when RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 6), 3) like '%K%' 
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 9), 3) like '%I%' 
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 9), 3) like '%3%'
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 9), 3) like '%4%' then 1 else 0 end DefMon6_9,
		cast(LEN(REPLACE(REPLACE(RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 9), 3), '/', ''), '.', '')) as float)/
		cast(LEN(RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 9), 3)) as float) OccpMon6_9,
		
		RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 12), 3) Mon9_12,
		case when RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 12), 3) like '%K%' 
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 12), 3) like '%I%' 
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 12), 3) like '%3%'
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 12), 3) like '%4%' then 1 else 0 end DefMon9_12,
		cast(LEN(REPLACE(REPLACE(RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 12), 3), '/', ''), '.', '')) as float)/
		cast(LEN(RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 12), 3)) as float) OccpMon9_12,
		
		RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 15), 3) Mon12_15,
		case when RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 15), 3) like '%K%' 
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 15), 3) like '%I%' 
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 15), 3) like '%3%'
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 15), 3) like '%4%' then 1 else 0 end DefMon12_15,
		cast(LEN(REPLACE(REPLACE(RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 15), 3), '/', ''), '.', '')) as float)/
		cast(LEN(RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 15), 3)) as float) OccpMon12_15,
		
		RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 18), 3) Mon15_18,
		case when RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 18), 3) like '%K%' 
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 18), 3) like '%I%' 
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 18), 3) like '%3%'
		or RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 18), 3) like '%4%' then 1 else 0 end DefMon15_18,
		cast(LEN(REPLACE(REPLACE(RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 18), 3), '/', ''), '.', '')) as float)/
		cast(LEN(RIGHT(LEFT(RIGHT(t3.Performance, len(t3.Performance)-(DATEDIFF(month,'2010-01-01', b.CreatedDateTime)+1)), 18), 3)) as float) OccpMon15_18

		from dbo.CustomerTransaction b
		left join
			...
			from
				...
				from 
					...
					from dbo.CustomerTransactionSummary a, dbo.Type b, dbo.CustomerTransaction c
					where a.CustomerId=c.CustomerId
					and c.Id in (select distinct CustomerTransactionId from dbo.KKB_15K_Part1)   
					and a.CreditTypeId=b.Id) t ) t2
			group by CustomerId, Description) t3
		on b.CustomerId = t3.CustomerId
		where b.Id in (select distinct CustomerTransactionId from dbo.KKB_15K_Part1) ) t4
	group by CustomerTransactionId, TransTime, CustomerId) t_2

	on t_1.CustomerTransactionId = t_2.CustomerTransactionId ) t_last


	(select CustomerTransactionId from dbo.x_indus_badr_final)
